...
export const companyDeepDives = [
  // ── Spring ──────────────────────────────────────────────────────────────
  {
    ...
    roleEs: 'Desarrollador de Software',
    domainEn:
      'Built and maintained a complete field sales automation ecosystem for consumer goods companies across Latin America. Sales reps carried Windows Mobile PDAs on daily routes, taking orders, collecting payments, verifying inventory, and syncing data to back-office servers that fed into SAP and other ERPs.',
    domainEs:
      'Construyo y mantuvo un ecosistema completo de automatizacion de ventas en campo para empresas de bienes de consumo en Latinoamerica. Los representantes de ventas portaban PDAs Windows Mobile en rutas diarias, tomando pedidos, cobrando pagos, verificando inventario y sincronizando datos con servidores back-office alimentando SAP y otros ERPs.',
    products: [
      {
  ...
  // ── DigitalData ─────────────────────────────────────────────────────────
  {
    ...
    techTable: [
      ...
      { layer: 'Computer Vision', technology: 'OpenCV (deskewing, noise removal, binarization), Tesseract OCR' },
      { layer: 'Frontend', technology: 'Angular 12.2 (RxJS, Material), SignalR (real-time updates)' },
      { layer: 'Database', technology: 'SQL Server' },
    ...
    complexityEn: [
      'Multi-model inference: Classification model picks document type, then type-specific extraction model locates and reads fields using OCR + NLP',
      'Computer vision preprocessing: OpenCV pipeline — deskew, denoise, binarize — to clean scanned images before OCR',
      'gRPC service: Protocol Buffers proto3 definition for high-performance search service',
      'Kafka event streaming: Consumer/producer architecture for async document processing and inter-service communication',
      'Multi-version product evolution: Three major DDOC versions (v3 → v4 → v5) with progressive architecture modernization',
      '50+ client deployments: Multi-tenant with per-client configurations, collection schemas, and workflow rules',
    ],
    complexityEs: [
      'Inferencia multi-modelo: Modelo de clasificacion selecciona tipo de documento, luego modelo de extraccion especifico localiza y lee campos usando OCR + NLP',
      'Preprocesamiento de vision por computadora: Pipeline de OpenCV — alinear, limpiar, binarizar — para limpiar imagenes escaneadas antes de OCR',
      'Servicio gRPC: Definicion Protocol Buffers proto3 para servicio de busqueda de alto rendimiento',
      'Streaming de eventos Kafka: Arquitectura consumidor/productor para procesamiento asincrono de documentos y comunicacion entre servicios',
      'Evolucion multi-version del producto: Tres versiones mayores de DDOC (v3 → v4 → v5) con modernizacion progresiva de arquitectura',
      'Mas de 50 despliegues de clientes: Multi-tenant con configuraciones, esquemas de coleccion y reglas de flujo de trabajo por cliente',
    ],
  },
